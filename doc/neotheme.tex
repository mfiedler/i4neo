...
%% based on the neo template by Matthias Vogelgesang et al.
...
\ifboolexpr{bool {xetex} or bool {luatex}}{
  ...
  \IfFileExists{../fonts/FiraSans-Regular.otf}{
    ...
    \PackageWarning{beamerthemeneo}{%
      Missing Package fonts directory - using system fonts.%
    }
  }
  \setmonofont
  ...
  \typeout{%
    You need to compile this with XeLaTeX or LuaLaTeX for correct Fira fonts.%
  }
}
...
\usepackage{xcolor}
\definecolor{nDarkBrown}{HTML}{604c38}
\definecolor{nDarkTeal}{HTML}{23373b}
\definecolor{nLightBrown}{HTML}{EB811B}
\definecolor{nLightGreen}{HTML}{14B03D}
\definecolor{nBackground}{HTML}{FFFFFF}

\usepackage{listings}
\lstset{%
  language=[LaTeX]{TeX},
  basicstyle=\ttfamily,
  keywordstyle=\color{nLightBrown}\bfseries,
  commentstyle=\color{nLightGreen},
  stringstyle=\color{nLightGreen},
  backgroundcolor=\color{nBackground},
  numbers=none,
  ...
  aboveskip=1em,
  belowskip=1em,
  morekeywords={usetheme,institute,maketitle,@neo@titleformat,%
                plain,setbeamercolor,neoset,setsansfont,setmonofont},
}
\lstMakeShortInline|
\usepackage{metalogo}

\usepackage[colorlinks=true,
            linkcolor=nLightBrown,
            menucolor=nLightBrown,
            pagecolor=nLightBrown,
            urlcolor=nLightBrown]{hyperref}

\newcommand{\DescribeOption}[4]{
  \DescribeMacro{#1}
  \begin{minipage}[t]{\textwidth}
    \textit{\textbf{\textcolor{nLightGreen}{#2}}}\dotfill\,#3\par
    \begingroup
    \vspace{0.5em}#4\par
    \endgroup
  \end{minipage}
...
\usepackage{readprov}
\ReadPackageInfos{beamerthemeneo}
...
for anyone to use -- it is based on the \href{https://github.com/matze/mtheme}
{metropolis} theme by \href{mailto:matthias.vogelgesang@gmail.com}{Matthias
Vogelgesang} \href{https://github.com/matze/mtheme/graphs/contributors}
{and many other of contributors}. It tries to minimize noise and maximize space
for content; the only visual flourish it offers is an (optional) progress bar
added to each slide or to the section slides.
...
\begin{description}
  ...
  \item[Compile the style files] by running |make sty| inside the downloaded
    directory. (Or run \LaTeX{} directly on |source/neotheme.ins|.)
  ...
  \item[Use the theme for your presentation] by declaring
    |\usetheme{neo}| in the preamble of your Beamer document.
\end{description}
...
\begin{lstlisting}
$ pandoc -t beamer --latex-engine=xelatex -V theme:neo -o output.pdf input.md
\end{lstlisting}
...
\begin{lstlisting}
\usetheme[option1=value1, option2=value2, ...]{neo}
\end{lstlisting}

Options can be changed at any time --- even mid-presentation! --- with the
|\neoset| macro.
\begin{lstlisting}
\neoset{option1=newvalue1, option2=newvalue2, ...}
\end{lstlisting}
...
\DescribeOption{startsection}{hide, show}{show}{
  Controls whether a section page as the very first slide is shown (|show|) or hidden (|hide|).
}

\DescribeOption{sectionaftertoc}{hide, show}{show}{
  Controls whether a section page directly after a table of contents frame is shown (|show|) or hidden (|hide|).
}
...
\DescribeOption{background}{dark, light, white}{light}{
  Provides the option to have a dark background and light foreground instead
  of the reverse. White is similar to light but uses a pure white background.
}
...
For example, overriding the color theme as follows may not work as expected because |\usetheme{neo}| loads the \themename color theme, which
...
\DocInput{beamerthemeneo.dtx}
\DocInput{beamerinnerthemeneo.dtx}
\DocInput{beamerouterthemeneo.dtx}
\DocInput{beamerfontthemeneo.dtx}
\DocInput{beamercolorthemeneo.dtx}
